...
BT_GATT_SERVICE_DEFINE(
    sbr_service, BT_GATT_PRIMARY_SERVICE(BT_UUID_SBR_SERVICE),

    BT_GATT_CHARACTERISTIC(BT_UUID_SBR_ACCEL, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ, read_accel, NULL, &button_state),

    BT_GATT_CHARACTERISTIC(BT_UUID_SBR_GYRO, BT_GATT_CHRC_READ,
                           BT_GATT_PERM_READ, read_gyro, NULL, &button_state),
...
ssize_t read_accel(struct bt_conn *conn, const struct bt_gatt_attr *attr,
  ...
  // BT_GATT_CHARACTERISTIC() and stored in attr->user_data
  const char *value = attr->user_data;

  // LOG_DBG("Attribute read, handle: %u, conn: %p", attr->handle, (void
  // *)conn);

  if (sbr_cb.button_cb) {
    // Call the application callback function to update the get the current
    // value of the button
...
ssize_t update_control(struct bt_conn *conn, const struct bt_gatt_attr *attr,
                       ...
                       uint8_t flags) {
  // LOG_DBG("Attribute write, handle: %u, conn: %p", attr->handle, (void
  // *)conn);
...
bool sbr_accel_cb(void) { return false; }

bool sbr_gyro_cb(void) { return true; }
