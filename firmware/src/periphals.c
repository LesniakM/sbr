...
int configure_gpios() {
  ...
  // ret = gpio_pin_configure_dt(&mpu6500_address_pin, GPIO_OUTPUT_INACTIVE);
  /*if (ret != 0) {
      LOG_ERR("Error %d: failed to configure mpu6500 address pin %d\n", ret,
  mpu6500_address_pin.pin); return ret;
  }*/
  // ret = gpio_pin_configure_dt(&m_driver_12_sleep, GPIO_OUTPUT_ACTIVE);
  /*if (ret != 0) {
